...
- 💬 **提示词预设**：一键切换多套翻译风格
- 🌐 **中英双语界面** + 系统托盘最小化
- 🩹 **智能输入容错**：Base URL 自动格式化（自动过滤尾部斜杠与 `/v1`），自定义请求头实时 JSON 校验，数值与下拉框防滚轮误触
- 🔍 **便捷调试工具**：日志页支持实时检索过滤与一键复制，测试页采用左右双栏布局，支持 `Ctrl+Enter` 快捷键提交
...
1. 填 **Base URL**（SakuraLLM 或任何 OpenAI 兼容接口，如 `http://127.0.0.1:8080`，输入带 `/v1` 或尾部斜杠会自动格式化）
2. 填 **模型名称**
3. 设置 **本地监听端口**（默认 4000）
